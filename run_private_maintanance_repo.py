...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        scs_path = os.path.join(os.path.dirname(__file__), "studies", "Maintanance_tests", "scenarios")
        # Base Examples IRS only
        # ----------------------
        # h) Pooling with public charging infrastructure (low range vehicles)
        log_level = "info"
        cc = os.path.join(scs_path, "const_Maintanance_infra.csv")
        sc = os.path.join(scs_path, "scenario_maintanance_vs_nomaintanance.csv")
